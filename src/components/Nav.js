    ...
    <div className="fixed top-0 left-0 right-0 z-50 px-6 sm:px-8 pt-6 sm:pt-8 lg:px-12 xl:px-16 animate-fadeInDown safe-area-padding">
      <nav className="flex items-center justify-between py-3 px-6  sm:py-4 sm:px-8" aria-label="Global">
        ...
        <div className="flex lg:hidden">
          <button
            type="button"
            className="-m-2.5 inline-flex items-center justify-center rounded-xl p-3 text-white hover:text-primary transition-colors duration-200 touch-target"
        ...
        <div className="hidden lg:flex lg:gap-x-8 xl:gap-x-10 lg:items-center">
          {navigation.map((item) => (
            <HashLink
              ...
              to={item.linkUrl}
              smooth
              className="text-sm xl:text-base font-medium text-white hover:text-primary transition-colors duration-300 py-2"
            >
              {item.name}
            </HashLink>
          ))}
          <a href={Ashaz} download="Ashaz_Ahmed_Resume">
            <button
              ...
              className="ml-6 xl:ml-8 px-6 xl:px-8 py-2.5 xl:py-3 bg-primary text-white font-semibold rounded-full hover:bg-orange-dark transition-all duration-300 text-sm xl:text-base shadow-lg hover:shadow-xl transform hover:scale-105 palantir-glow-sm"
            >
              Resume
            </button>
          </a>
      ...
      <Dialog as="div" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
        <Dialog.Panel className="fixed inset-0 z-10 overflow-y-auto bg-white/95 backdrop-blur-md lg:hidden safe-area-padding">
          <div className="flex items-center justify-between px-6 sm:px-8 py-6 sm:py-8">
            ...
            <button
              type="button"
              className="rounded-xl p-3 text-white hover:text-primary transition-colors duration-200 touch-target"
              ...
              <div className="space-y-3 py-8">
                {navigation.map((item) => (
                  <HashLink
                    ...
                    to={item.linkUrl}
                    smooth
                    className="-mx-3 block rounded-2xl py-5 px-5 text-base sm:text-lg font-medium text-white hover:text-primary hover:bg-warm-gray transition-colors duration-200 touch-target"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {item.name}
                  </HashLink>
                ...
                <div className="pt-6">
                  <a href={Ashaz} download="Ashaz_Ahmed_Resume">
                    <button className="w-full px-6 py-4 bg-primary text-white font-semibold rounded-full hover:bg-orange-dark transition-all duration-300 text-base touch-target shadow-lg palantir-glow-sm">
                      Download Resume
                    </button>
                  </a>
                </div>
